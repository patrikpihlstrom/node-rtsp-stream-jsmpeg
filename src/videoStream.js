...
class VideoStream extends EventEmitter {
	constructor(options) {
		...
		this.server = null;
		this.muxerOptions = options.muxerOptions || [];

		this.stream2Socket();
	}

	stream2Socket() {
		let socketServer = new WebSocket.Server({port: this.wsPort, perMessageDeflate: false});
	...
	start() {
		this.mpeg1Muxer = new Mpeg1Muxer({url: this.url, port: this.port, muxerOptions: this.muxerOptions});
